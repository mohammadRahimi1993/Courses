# Academic Thesis Archive | Mohammad Rahimi

### 👋 About Me

* 🎓 **Candidate:** Mohammad Rahimi
* 🏫 **Affiliations:** 
    * [University of Isfahan](https://ui.ac.ir) | Department of Software Engineering
    * [Iran University of Science and Technology (IUST)](https://www.iust.ac.ir) | Noor Campus
* 🔬 **Research Focus:**
    * 🔗 **Inductive Link Prediction (ILP)**
    * 🧠 **Deep Learning** (Specializing in *Restricted Boltzmann Machines*)
* 🌐 **Website:** [programmingschool.ir](https://programmingschool.ir/)

---

## Overview
This archive contains the documentation and video presentations for my academic defense sessions. These presentations highlight the core research contributions made during my PhD and Master’s programs.

## Contents

### 1. PhD Dissertation
* **Proposal Title(2024):** Inductive link prediction based on constructing neighbors for unseen nodes
* **Video File/Link:** [https://www.aparat.com/v/yaw516n] 
* **Paper-1(2025):** PersianILP: Construction and Evaluation of a Standard Persian Dataset for Inductive Link Prediction
* **Video File/Link:** [https://www.aparat.com/v/npjg3k9]
* **Paper-2(2026):** ILP-SAGE: A Sampling-Based Framework for Scalable Inductive Link Prediction

### 2. Master's Thesis
* **Title(2021):** Hyperparameter Tuning of Deep Belief Networks using a Metaheuristic Approach
* **Video File/Link:** [https://www.aparat.com/v/jhgq1ym]

---
*Maintained by: Mohammad Rahimi*  
*Last Updated: July 2026*
